 ...
 public function index()
    {
        $skip = 0;
        $limit = 2000;
        $serverURL = 'http://172.16.1.252:7048/DynamicsNAV90/OData/Company(\'Muscat%20Duty%20Free\')';
        $usernamePassword = "mdfadminraja:Access@789789";

        $url = $serverURL . '/Inventory?$filter=Is_Sindbad_Item%20eq%20true&$format=json&$skip=' . $skip . '&$top=' . $limit;
    ...
    //    print_r($data);
            ...
            $products =  Product::find($value->Item_No);
            if (!empty($value->$products)) {
            if(!empty($value->Inventory) && !empty($value->Buffer_Stock)){
            $products->inventory = $value->Inventory;
            $products->buffer_stock = $value->Buffer_Stock;
            // $product->buffer_stock= $request->buffer_stock;
            $products->save();
    ...
    public function store(Request $request, $id){
        ...
        $products->buffer_stock = $request->input("buffer_stock");
        // $product->buffer_stock= $request->buffer_stock;
         $products->save();
            // $response_data = curl_exec($curl);
    ...
    public function getProductDetails($encProductId)
    {
        ...
        if(isset($productDetails->productId)){
           ...
           $optionalProduct = Product::leftjoin('brands', 'brands.id', '=', 'products.product_brand_id')
            ...
            $productCount = Helper::getItemInventory($productId);
            return view('productDetail',compact('productDetails', 'productImages', 'deliveryOptions' , 'availablecolors', 'availableSizes', 'optionalProduct', 'productCount'));
        }else{
            return redirect('');
        }
