...
class ProductController extends Controller
{
    public function index(Request $request)
    {
        ...
        $products = Product::leftjoin('brands', 'brands.id', '=', 'products.product_brand_id')
            ...
            ->where(['products.status'=>$type, 'product_images.is_prime'=>1, 'product_images.status'=>1])->Orwhere(['product_images.is_prime' => 1, 'product_images.status' => 1])->select('category_name', 'sub_category_name', 'brand_name', 'product_cost', 'product_color', 'product_size', 'products.status', 'products.id as productId', 'product_name', 'products.updated_at', 'products.is_prime', 'product_images.image_name')->get();
    ...
    {          
        ...
        if(!$edit){
            ...
            $newData = Product::where(['id'=>$productId])->first()->toArray();
            $oldData = [];
            $action = 'Create';
        }else{
            $oldData = Product::where(['id' => $productId])->first()->toArray();
            $createCategory = Product::where(['id'=>$productId])->update($insertData);
            $newData = Product::where(['id'=>$productId])->first()->toArray();
            $action = 'Update';
        }
        $updateLogs = Helper::storeLogs($oldData,  $newData, $action, 'Product');
        if($productId){
